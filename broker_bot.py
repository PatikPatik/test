...
CREATE TABLE IF NOT EXISTS executors(
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER,                    -- FK to users.id (nullable, до первого /start)
  pending_username TEXT,              -- если ещё не пришёл /start
  categories TEXT,                    -- "Экскаватор,Самосвал"
  city TEXT,
  lat REAL,
  lon REAL,
...
async def get_or_create_user(tg, role: Optional[str]=None) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
        # auto-link executor by pending_username if matches
        if tg.username:
            await db.execute(
...
(OFFER_RATE_TYPE, OFFER_RATE_VALUE, OFFER_COMMENT) = range(3,6)
...
async def main():
    ...
    start_conv = ConversationHandler(
        ...
        states={
            0: [MessageHandler(filters.TEXT & ~filters.COMMAND, role_selected)]  # ROLE_SEL
        },
        fallbacks=[]
    )
    req_conv = ConversationHandler(
        ...
        states={
            1: [MessageHandler(filters.TEXT & ~filters.COMMAND, cat_selected)],   # CAT_SEL
            2: [MessageHandler(filters.TEXT & ~filters.COMMAND, desc_input)],     # DESC_IN
            3: [MessageHandler(filters.LOCATION, location_received)],             # LOC_WAIT
            4: [MessageHandler(filters.TEXT & ~filters.COMMAND, radius_input)]    # RAD_IN
        },
        fallbacks=[]
    )
    ...
    app.add_handler(CallbackQueryHandler(on_offer_click, pattern=r"^offer:\\d+:\\d+$"))
    app.add_handler(CallbackQueryHandler(on_rate_type, pattern=r"^rt:(час|смена|объект)$"))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, on_rate_value), 4)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, on_offer_comment), 5)
    app.add_handler(CallbackQueryHandler(on_accept_offer, pattern=r"^accept_offer:\\d+$"))
    app.add_handler(CommandHandler("me", cmd_me))
    app.add_handler(CommandHandler("admin", cmd_admin))
    app.add_handler(MessageHandler(filters.LOCATION, on_location_reply))
